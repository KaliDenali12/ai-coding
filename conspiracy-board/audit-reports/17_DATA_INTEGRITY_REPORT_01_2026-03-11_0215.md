...
| # | Recommendation | Impact | Risk if Ignored | Worth Doing? | Status |
|---|---------------|--------|----------------|-------------|--------|
| 1 | Add request body size check independent of Content-Length header | Closes DI-002: Content-Length bypass. Could limit `request.text()` length before JSON parsing. | Low — Netlify has its own limits, and field-level validation caps useful payload size. | Only if time allows | **FIXED** in `fb82da4` |
| 2 | Add upper-bound length limits to AI response fields | Closes DI-004: Unbounded string lengths from AI. e.g., title ≤ 100 chars, teaser ≤ 500, briefing ≤ 5000. | Low — only affects UI layout if AI returns unexpectedly long text. | Probably | **FIXED** in `fb82da4` |
| 3 | Validate `classification_level` against known values | Closes DI-005: Accept only the 4 expected values. | Low — purely cosmetic field, no logic depends on it. | Only if time allows | Not pursued — coupling cost outweighs benefit |
| 4 | Run example chip values through `checkInputs()` at submit time | Closes DI-001: Ensures hardcoded values go through full validation pipeline. | Low — values are developer-controlled and currently safe. | Only if time allows | Not pursued — marginal benefit |
...
*Report generated by automated audit. Recommendations #1 and #2 were implemented and committed (`fb82da4`). All 267 tests pass after changes.*
